...
<template>
  <div class="container">
    <a
      v-if="monsterList.value"
      v-for="info in monsterList.value"
      :href="info[8]"
      target="_blank"
      class="monster-card"
    >
      ...
      <div class="info">
        ...
        <br />
        距離 <span style="color: #a95620">{{ info[3] }}</span> 約 {{ Math.floor(info[9]) > 0 ?  info[9] + ' 公里' : parseInt(info[9].toString().slice(-3)) + ' 公尺' }}
      </div>
    </a>
